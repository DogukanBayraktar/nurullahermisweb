                    ...
                    <div key={si} className="bg-white border border-slate-100 rounded-xl p-3 space-y-3 relative group">
                      ...
                      <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-3 space-y-3">
                        ...
                        {sub.patients?.map((patient, pi) => (
                          <div key={pi} className="rounded-xl border border-slate-100 bg-white p-3 space-y-3 relative group">
                            <div className="flex items-center gap-2">
                              <input value={patient.hastaAdi}
                                onChange={(e) => setForm(lang, { treatment: getForm(lang).treatment.map((s, j) => j === i ? { ...s, submethods: s.submethods?.map((sm, x) => x === si ? { ...sm, patients: sm.patients?.map((p, y) => y === pi ? { ...p, hastaAdi: e.target.value } : p) } : sm) } : s) })}
                                placeholder={lang === 'tr' ? 'Hasta adı (örn. Burak İpekçi)' : 'Patient name (e.g. Burak İpekçi)'} className={inputCls} />
                              <button type="button"
                                onClick={() => setForm(lang, { treatment: getForm(lang).treatment.map((s, j) => j === i ? { ...s, submethods: s.submethods?.map((sm, x) => x === si ? { ...sm, patients: sm.patients?.filter((_, y) => y !== pi) } : sm) } : s) })}
