...
export default function AboutClient({ aboutData }: { aboutData: any }) {
  ...
  const content = aboutData[currentLang] || aboutData['tr'];

  const education = content.education.items as TimelineItem[];
  const experience = content.experience.items as ExperienceItem[];
  const publications = content.publications.items as PublicationItem[];
  const certificates = content.certificates.items as CertificateItem[];
  const congresses = content.congresses.items as CongressItem[];
  const memberships = content.memberships.items as string[];
  const books = content.books.items as BookItem[];
  const expertiseItems = content.expertise.items as ExpertiseItem[];

  // Hero badges can still come from i18n as they are static labels for now, 
  // or we can just use a fixed set if preferred.
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
        ...
        <div className="container relative z-10 mx-auto max-w-6xl px-4">
          <div className="flex flex-col items-center gap-12 lg:flex-row lg:items-start lg:gap-20">
            ...
            <FadeIn direction="up" delay={0.2} className="w-full lg:w-7/12">
              <p className="mb-4 block text-xs font-bold uppercase tracking-[0.18em] text-blue-600">{content.hero.badge}</p>
              <h1 className="mb-3 text-4xl font-extrabold tracking-tight text-slate-900 md:text-5xl">{content.hero.name}</h1>
              <p className="mb-8 text-xl font-medium text-slate-500">{content.hero.title}</p>
              <div className="mb-8 space-y-4 text-[1.05rem] leading-relaxed text-slate-600">
                <p>{content.hero.bio1}</p>
                <p>{content.hero.bio2}</p>
                <p>{content.hero.bio3}</p>
              </div>
      ...
      <section className="bg-white py-20">
        <div className="container mx-auto max-w-6xl px-4">
          <div className="mb-16 text-center">
            <p className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-blue-600">{content.education.badge}</p>
            <h2 className="mb-4 text-3xl font-extrabold text-slate-900 md:text-4xl">{content.education.title}</h2>
          </div>

          <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
            <div>
              <h3 className="mb-6 flex items-center gap-2.5 text-lg font-bold text-slate-900">
                <GraduationCap className="h-5 w-5 text-blue-600" /> {content.education.columnTitle}
              </h3>
            ...
            <div>
              <h3 className="mb-6 flex items-center gap-2.5 text-lg font-bold text-slate-900">
                <Building className="h-5 w-5 text-blue-600" /> {content.experience.columnTitle}
              </h3>
              <div className="space-y-0">
          ...
          <div className="mt-12 pt-10 border-t border-slate-100">
            <h3 className="mb-6 flex items-center gap-2.5 text-lg font-bold text-slate-900">
              <Award className="h-5 w-5 text-blue-600" /> {content.certificates.title}
            </h3>
      ...
      <section className="border-y border-slate-100 bg-slate-50 py-20">
        <div className="container mx-auto max-w-6xl px-4">
          <div className="mb-14 text-center">
            <p className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-blue-600">{content.expertise.badge}</p>
            <h2 className="mb-4 text-3xl font-extrabold text-slate-900 md:text-4xl">{content.expertise.title}</h2>
            <p className="mx-auto max-w-2xl text-lg text-slate-500">{content.expertise.subtitle}</p>
          </div>
      ...
      <section className="bg-white py-20">
        <div className="container mx-auto max-w-6xl px-4">
          <div className="mb-14 text-center">
            <p className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-blue-600">{content.publications.badge}</p>
            <h2 className="mb-4 text-3xl font-extrabold text-slate-900 md:text-4xl">{content.publications.title}</h2>
            <p className="mx-auto max-w-2xl text-lg text-slate-500">{content.publications.subtitle}</p>
          </div>
          ...
          <div className="mt-12 border-t border-slate-100 pt-10">
            <h3 className="mb-6 flex items-center gap-2.5 text-lg font-bold text-slate-900">
              <BookOpen className="h-5 w-5 text-blue-600" /> {content.books.title}
            </h3>
          ...
          <div className="grid gap-16 lg:grid-cols-2">
            <div>
              <h2 className="mb-8 flex items-center gap-2.5 text-2xl font-extrabold text-slate-900">
                <Globe className="h-6 w-6 text-blue-600" /> {content.congresses.title}
              </h2>
            ...
            <div>
              <h2 className="mb-8 flex items-center gap-2.5 text-2xl font-extrabold text-slate-900">
                <Users className="h-5 w-5 text-blue-600" /> {content.memberships.title}
              </h2>
